...
// social media
if ( ! function_exists( 'photomedia_social' ) ) {
	function photomedia_social( $args = array()  ){
		...
		$url = photomedia_opt('photomedia_social_url');
...
function photomedia_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'photomedia' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'photomedia' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'photomedia' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function photomedia_set_post_views($postID) {
    $count_key = 'photomedia_post_views_count';
...
// blog post categoty 
function photomedia_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'photomedia' ) );
